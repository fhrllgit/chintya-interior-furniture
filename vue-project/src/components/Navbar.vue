    ...
    <header class="relative">
        <transition name="slide-down">
            <div v-show="showNavbar"
                class="flex  bg-[#2C1810] md:bg-[#2C1810] text-white  fixed top-0 z-90 h-16 md:h-20 py-3 px-4 sm:px-6 md:px-10 lg:px-20 xl:px-32 w-full items-center justify-between transition-colors duration-300">
                <div>
                ...
                <nav class="lg:flex hidden items-center gap-6 lg:gap-8 text-sm lg:text-sm">
                    <RouterLink v-for="item in arrNavItems" :to="item.to" :key="item">
                        <button class="cursor-pointer hover:text-[#2C1810] transition-colors">{{ item.name }}</button>
                    </RouterLink>
                </nav>
                <div
                    class="lg:flex hidden items-center justify-center border-2 border-[#2C1810] hover:bg-[#2C1810] hover:text-white hover:border-[#2C1810] transition-all w-36 lg:w-40 text-xs lg:text-sm cursor-pointer font-semibold text-center py-2 rounded-3xl">
                    <button class="cursor-pointer">Konsultasi</button>
                </div>
                <button @click="toggleMenu" class="lg:hidden flex p-2 -mr-2" aria-label="Toggle menu">
        ...
        <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-4"
            ...
            <div v-if="isMenuOpen" class="flex lg:hidden bg-white shadow-lg fixed top-16 left-0 right-0 z-10 w-full">
                <nav class="flex flex-col w-full items-center justify-center space-y-4 py-6 px-4">
                    ...
                    <button
                        class="cursor-pointer w-full max-w-xs border-2 border-[#2C1810] bg-[#2C1810] text-white font-semibold py-3 rounded-3xl mt-4 hover:bg-[#6d5a43] transition-colors">Konsultasi</button>
                </nav>
            </div>
        </transition>
    </header>
...
const arrNavItems = [
    ...
    { name: 'Tentang', to: { path: '/', hash: '#tentang-kami' } },
