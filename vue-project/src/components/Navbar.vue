<template>
    <header class="relative">
            ...
            <div v-show="showNavbar"
                ...
                <nav class="lg:flex hidden items-center gap-6 lg:gap-8 text-sm lg:text-sm">
                    <RouterLink v-for="item in arrNavItems" :to="item.to" :key="item">
                        <button style="font-weight: 500;"
                            class="cursor-pointer text-[#b8b7b7] hover:text-white transition-colors">{{ t(item.key) }}</button>
                    </RouterLink>
                </nav>
                ...
                <div class="flex items-center justify-center gap-5">
                    ...
                    <button @click="toggleMenu"
                        class="lg:hidden border-2 border-[#ffffff63] hover:border-white hover:bg-[#ffffff23] cursor-pointer rounded-xl flex p-2 -mr-2"
                        aria-label="Toggle menu">
                        <svg v-if="!isMenuOpen" class="size-6 text-white cursor-pointer"
        ...
        <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-4"
            ...
            <div v-if="isMenuOpen"
                class="flex flex-col lg:hidden border-t-2 border-t-[#5A3A2A] bg-[#2C1810] shadow-lg fixed mt-22 left-0 right-0 z-50 w-full">
        ...
        <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-4"
            ...
            leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-4">
            <div v-if="isSearch"
                class="fixed gap-5 flex px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40  mt-22 z-30 border-t-2 border-t-[#5A3A2A] bg-[#2C1810] w-full items-center justify-center h-25">
                <div
